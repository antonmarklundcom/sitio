# Phase S3 — Theme `belleza`. Sonnet session. Lane 2, parallel with S1–S5.
...
Owns: `src/themes/belleza/**`, `docs/log/S3.md`.
...
a `/* == S3 == */` block at the end of `src/themes/theme.css` only if a shared
primitive is genuinely missing (prefer `belleza.css`).
...
- Design direction for `belleza` is fixed in plan §6.2. Different section map
...
- QA gate once, after the last CSS change: `npm run theme:preview && npm run
  theme:shots belleza`; fix overflow at 360/768/1280 for all four variants.
...
Exit: `isThemeBuilt("belleza")` true; QA gate green for v1–v4; registry section
