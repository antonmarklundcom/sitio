# Phase S6 — Link pass. Sonnet session. Sequential, after S1, S2, S5, S7 are merged.
...
`src/themes/{servicios,gastronomia,salud}/*-theme.tsx` (four themes total, plan §1.12),
...
(smoke or screenshot); QA gate green ×4; `presentationFor` covers every
category (S7's test still green on `main`); smoke green end to end;
