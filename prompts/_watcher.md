...
1. For each lane 2 phase (S1, S2, S5, S7 — S3 and S4 are cancelled, ignore
   them) decide its state:
...
2. Actions, while fewer than 4 phases are running:
   - stalled → re-spawn it (`create_session`, `model` `claude-sonnet-5`,
     for S7 `claude-opus-5`, prompt `Read prompts/<file>.md in this repo
     and execute it.`). Prompts are re-runnable. Never touch a branch named
     `phase/S7-astra`; it is Anton's manual bake-off branch.
   - not started → spawn it, same call.
   - PR green and mergeable but its session is gone → merge it, add the §9
     line if missing.
3. When S1, S2, S5 and S7 are all merged and `phase/S6` does not exist → spawn S6
