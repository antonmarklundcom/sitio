...
type ResetClaims = { purpose: typeof PURPOSE; userId: number; email: string; expiresAt: number };
...
export function createResetToken(userId: number, email: string, now: number): string {
  if (!Number.isSafeInteger(userId) || userId <= 0 || !email || !Number.isSafeInteger(now)) {
    throw new Error("Invalid reset claims.");
  }
  const claims: ResetClaims = { purpose: PURPOSE, userId, email, expiresAt: now + TTL };
